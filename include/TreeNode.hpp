...
public:
    ...
    //key functions
    // It returns NULL if key already exists or if the key is inserted successfully without causing an overflow.
    virtual TreePtr insert_key(const Key& key, const RecordPtr& record_ptr) {return NULL_PTR;}
    virtual void redistributeData(TreeNode *from, TreeNode *to, int fromIndex, int toIndex) {};
    virtual void merge_nodes(TreeNode*) {};
